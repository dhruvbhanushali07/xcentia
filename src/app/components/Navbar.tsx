...
const NAV_LINKS = [["Fragrances", "/shop"]] as const;
      ...
      `}</style>
          ...
          ${
			...
			>
				...
				<Link
					href="/"
					className="flex items-center gap-3 no-underline text-[#211911]"
				>
					<svg
              ...
              ${
				...
				<div className="flex items-center gap-2">
					...
					<button
						onClick={handlePopup}
						className="p-2 text-[#211911]/40 hover:text-[#211911] transition-colors duration-200 rounded-full hover:bg-black/4"
					>
						<svg
							className="w-4 h-4"
							fill="none"
			...
			{/* Mobile menu */}
			<div
				className={`md:hidden fixed inset-0 z-40 bg-[#f8f7f6] transition-all duration-300 flex flex-col
        ...
        ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
			...
			{profilePopup ? (
				<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center backdrop-blur-sm">
					<div className="bg-[#f8f7f6] rounded-lg shadow-2xl w-full max-w-md mx-4 p-8 relative">
						{/* Close button */}
						<button
							onClick={() => setprofilePopup(false)}
							className="absolute top-6 right-6 text-[#211911]/40 hover:text-[#211911] transition-colors"
						>
							<svg
								className="w-6 h-6"
								fill="none"
